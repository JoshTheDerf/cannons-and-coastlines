...
<template>
  <section id="signup" class="py-20 px-4 band-deck">
    <div class="container mx-auto grid lg:grid-cols-[1.4fr_1fr] gap-10">
      ...
      <div class="card-parchment self-start p-6 flex gap-5 items-start">
        <div class="flex-1 min-w-0">
          <h3 class="font-display text-lg">{{ data.discord.title }}</h3>
          <hr class="rule-gold mt-2">
          <p class="mt-2 text-sm muted">{{ data.discord.body }}</p>
          <UButton :to="data.discord.url" target="_blank" icon="i-lucide-message-circle" color="primary" size="md" class="mt-4">
            Join the Discord
          </UButton>
        </div>
        <img :src="data.discord.qr" alt="Discord invite QR" class="size-28 rounded-sm bg-white p-1 border border-[#3a2f22]/40 shrink-0">
      </div>
    </div>
  </section>
</template>
